...
pub fn normal_level(p: &mut RapierData, gpu: &WgpuData, pr: &mut PlaneRenderer, res: &ResourceManager) -> anyhow::Result<Level> {
...
pub fn long_tunnel(p: &mut RapierData, gpu: &WgpuData, pr: &mut PlaneRenderer, res: &ResourceManager) -> anyhow::Result<Level> {
    let gf = res.textures.get("gf").ok_or(anyhow!("NO TEXTURE"))?;
    let bf = res.textures.get("bf").ok_or(anyhow!("NO TEXTURE"))?;
    let mut gfs = pr.create_plane(&gpu.device, Some(&gf.view));
...
pub fn fat_tunnel(p: &mut RapierData, gpu: &WgpuData, pr: &mut PlaneRenderer, res: &ResourceManager) -> anyhow::Result<Level> {
    let gf = res.textures.get("gf").ok_or(anyhow!("NO TEXTURE"))?;
    let bf = res.textures.get("bf").ok_or(anyhow!("NO TEXTURE"))?;
    let pf = res.textures.get("pf").ok_or(anyhow!("NO TEXTURE"))?;
    let mut gfs = pr.create_plane(&gpu.device, Some(&gf.view));
